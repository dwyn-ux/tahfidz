...
const Store = (() => {
  ...
  function checkSetoranTerlewat() {
    ...
    db.santri.filter(s => s.status === 'Aktif').forEach(s => {
      // periksa hari kerja dalam window 7 hari terakhir hingga kemarin
      // (cukup untuk menangkap setoran terlewat tanpa membanjiri notif)
        ...
        if (isHariKerja(cek)) {
          ...
          if (!sudah(s.id, tgl)) {
            ...
            if (!adaSetoran) {
              ...
              const uUser = uObj ? db.users.find(u => u.role === 'ustadz' && u.refId === uObj.id) : null;
              ...
              if (wUser && !db.notifikasi.some(n => n.userId === wUser.id && n.pesan === pesan)) addNotif(wUser.id, 'wali', pesan);
              if (uUser && !db.notifikasi.some(n => n.userId === uUser.id && n.pesan === pesanU)) addNotif(uUser.id, 'ustadz', pesanU);
              tandai(s.id, tgl);
            }
          }
        }
